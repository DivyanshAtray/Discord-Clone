...
function createAttachmentElement(url, type) {
    let element;
    
    // If it's an image, create an <img> tag with a click listener for the modal
    if (type.startsWith("image/")) {
        ...
        element.onclick = () => openMediaModal(url, 'image');
    } 
    // If it's a video, create a <video> tag
        ...
        element.style.borderRadius = "8px";
        // Clicking the video also opens the large viewer
        element.onclick = () => openMediaModal(url, 'video');
    } 
    // For other files (PDFs, etc.), create the download layout you already had
    else {
        element = document.createElement("div");
        const filename = url.split('/').pop();
        element.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px; background: #2f3136; padding: 10px; border-radius: 8px;">
                <div style="color: #fff;">
